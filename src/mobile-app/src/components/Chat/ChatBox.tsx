...
import {conversationActions, sendMessage} from '../../stores/conversation';
...
const ChatBox: React.FC<IProps> = ({conversation}) => {
  ...
  const conversationMember = members.find(
    o => o.userMember.userId === userInfo?.userId,
  );
  ...
  const onMessageSeen = useCallback(async () => {
    ...
    if (!seenDate || compareDate(seenDate, lastMessageDate) === -1) {
      //seenDate < lastMessageDate
      console.log('updateMemberSeenDate');
      dispatch(
        conversationActions.updateMemberSeenDate({
          conversationId: conversation.id,
  ...
  let content: ReactElement<any, any> = conversation ? (
    <>
      ...
      {selectedMessage && (
        <ActionToolBar
          isOwnerMessage={
            userInfo?.userId === selectedMessage.userSender.userId
          }
          userRole={userRole}
