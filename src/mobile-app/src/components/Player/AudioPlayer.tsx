...
const AudioPlayer: React.FC<AudioPlayerProps> = ({
  ...
  useEffect(() => {
    ...
    const remainTimeStr = audioRecorderPlayer.mmss(
      Math.floor(durationMiliSeconds / 1000),
    );
  ...
  const onPlay = useCallback(async () => {
    ...
    try {
      ...
      audioRecorderPlayer.addPlayBackListener((e: PlayBackType) => {
        ...
        const remainTimeStr = audioRecorderPlayer.mmss(
          Math.floor(remainTimeMiliSeconds / 1000),
        );
  ...
  const onStop = useCallback(async () => {
    ...
    try {
      if (durationRef.current) {
        setTime(
          audioRecorderPlayer.mmss(Math.floor(durationRef.current / 1000)),
        );
      }
      audioRecorderPlayer.removePlayBackListener();
      await audioRecorderPlayer.stopPlayer();
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity style={styles.iconContainer} onPress={onTogglePlay}>
