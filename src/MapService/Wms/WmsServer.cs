...
namespace MapService.Wms
{
    ...
    public static class WmsServer
    {
        ...
        public static Task ParseQueryString(Map map, Capabilities.WmsServiceDescription description, int pixelSensitivity, InterSectDelegate intersectDelegate, HttpContext context)
        {
            _intersectDelegate = intersectDelegate;
            if (pixelSensitivity > 0)
                _pixelSensitivity = pixelSensitivity;
            return ParseQueryString(map, description , context);
        }
        ...
        /// <param name="description">Description of map service</param>
        /// <param name="context">The context the <see cref="WmsServer"/> is running in.</param>
        public static async Task ParseQueryString(Map map, Capabilities.WmsServiceDescription description, HttpContext context)
        {
            ...
            Microsoft.Extensions.Primitives.StringValues REQUESTVal;
            if (!context.Request.Query.TryGetValue("REQUEST",out REQUESTVal))
            {
                await WmsException.ThrowWmsException("Required parameter REQUEST not specified", context);
                return;
            }
            //Check if version is supported
            Microsoft.Extensions.Primitives.StringValues VERSIONVal;
            if (context.Request.Query.TryGetValue("VERSION", out VERSIONVal))
            {
                if (String.Compare(VERSIONVal.ToString(), "1.3.0", ignoreCase) != 0)
                {
                    await WmsException.ThrowWmsException("Only version 1.3.0 supported", context);
                    return;
                }
            }
            else
            ...
            {
                if (String.Compare(REQUESTVal, "GetCapabilities", ignoreCase) != 0)
                {
                    await WmsException.ThrowWmsException("VERSION parameter not supplied", context);
                    return;
                }
            }

            //If Capabilities was requested
            if (String.Compare(REQUESTVal.ToString(), "GetCapabilities", ignoreCase) == 0)
            {
                //Service parameter is mandatory for GetCapabilities request
                Microsoft.Extensions.Primitives.StringValues SERVICEVal;
                if (!context.Request.Query.TryGetValue("SERVICE",out SERVICEVal))
                {
                    await WmsException.ThrowWmsException("Required parameter SERVICE not specified", context);
                    return;
                }

                if (String.Compare(SERVICEVal.ToString(), "WMS", StringComparison.InvariantCulture/*IgnoreCase*/) != 0)
                    await WmsException.ThrowWmsException(
                        "Invalid service for GetCapabilities Request. Service parameter must be 'WMS'", context);

                XmlDocument capabilities = ServerCapabilities.GetCapabilities(map, description,context);
                context.Response.Clear();
                context.Response.ContentType = "text/xml";
                XmlWriter writer = XmlWriter.Create(context.Response.Body, new XmlWriterSettings
                {
                    Async = true,
                    CloseOutput = false,
            ...
            else if (String.Compare(REQUESTVal, "GetFeatureInfo", ignoreCase) == 0) //FeatureInfo Requested
            {
                StringValues LAYERSval;
                if (!context.Request.Query.TryGetValue("LAYERS", out LAYERSval))
                {
                    await WmsException.ThrowWmsException("Required parameter LAYERS not specified", context);
                    return;
                }
                StringValues STYLESval;
                if (!context.Request.Query.TryGetValue("STYLES", out STYLESval))
                {
                    await WmsException.ThrowWmsException("Required parameter STYLES not specified", context);
                    return;
                }
                StringValues CRSval;
                if (!context.Request.Query.TryGetValue("CRS", out CRSval))
                {
                    await WmsException.ThrowWmsException("Required parameter CRS not specified", context);
                    return;
                }
                if (CRSval.ToString() != "EPSG:" + map.Layers[0].TargetSRID)
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidCRS, "CRS not supported", context);
                    return;
                }
                StringValues BBOXval;
                if (!context.Request.Query.TryGetValue("BBOX", out BBOXval))
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue,
                                                   "Required parameter BBOX not specified", context);
                    return;
                }
                StringValues WIDTHval;
                if (!context.Request.Query.TryGetValue("WIDTH", out WIDTHval))
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue,
                                                   "Required parameter WIDTH not specified", context);
                    return;
                }
                StringValues HEIGHTval;
                if (!context.Request.Query.TryGetValue("HEIGHT", out HEIGHTval))
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue,
                                                   "Required parameter HEIGHT not specified", context);
                    return;
                }
                StringValues FORMATval;
                if (!context.Request.Query.TryGetValue("FORMAT", out FORMATval))
                {
                    await WmsException.ThrowWmsException("Required parameter FORMAT not specified", context);
                    return;
                }
                StringValues QUERY_LAYERSval;
                if (!context.Request.Query.TryGetValue("QUERY_LAYERS", out QUERY_LAYERSval))
                {
                    await WmsException.ThrowWmsException("Required parameter QUERY_LAYERS not specified", context);
                    return;
                }
                StringValues INFO_FORMATval;
                if (!context.Request.Query.TryGetValue("INFO_FORMAT", out INFO_FORMATval))
                {
                    await WmsException.ThrowWmsException("Required parameter INFO_FORMAT not specified", context);
                    return;
                }

                StringValues Xval, Ival;
                if (!context.Request.Query.TryGetValue("X", out Xval) && !context.Request.Query.TryGetValue("I", out Ival))
                ...
                {
                    await WmsException.ThrowWmsException("Required parameter I not specified", context);
                    return;
                }

                StringValues Yval, Jval;
                if (!context.Request.Query.TryGetValue("Y", out Yval) && !context.Request.Query.TryGetValue("J", out Jval))
                {
                    await WmsException.ThrowWmsException("Required parameter J not specified", context);
                    return;
                }
                //sets the map size to the size of the client in order to calculate the coordinates of the projection of the client
                try
                {
                    map.Size = new Size(Convert.ToInt16(WIDTHval),
                                        Convert.ToInt16(HEIGHTval));
                }
                catch
                {
                    await WmsException.ThrowWmsException("Invalid parameters for HEIGHT or WITDH", context);
                    return;
                }
                ...
                if(!context.Request.Query.TryGetValue("bbox", out bboxval))
                {
                    await WmsException.ThrowWmsException("Invalid parameter BBOX", context);
                    return;
                }
                var bbox = ParseBBOX(bboxval, map.Layers[0].TargetSRID == 4326);
                if (bbox == null)
                {
                    await WmsException.ThrowWmsException("Invalid parameter BBOX", context);
                    return;
                ...
                if (Xval.ToString()!="")
                    try
                    {
                        x = Convert.ToSingle(Xval);
                    }
                    catch
                    {
                        await WmsException.ThrowWmsException("Invalid parameters for X", context);
                        return;
                    }
                if (Ival.ToString()!="")
                    try
                    {
                        x = Convert.ToSingle(Ival);
                    }
                    catch
                    {
                        await WmsException.ThrowWmsException("Invalid parameters for I", context);
                        return;
                    }
                ...
                if (Yval.ToString() != "")
                    try
                    {
                        y = Convert.ToSingle(Yval);
                    }
                    catch
                    {
                        await WmsException.ThrowWmsException("Invalid parameters for Y", context);
                        return;
                    }
                if (Jval.ToString() != "")
                    try
                    {
                        y = Convert.ToSingle(Jval);
                    }
                    catch
                    {
                        await WmsException.ThrowWmsException("Invalid parameters for J", context);
                        return;
                    }
                ...
                try
                {
                    StringValues FEATURE_COUNTval;
                    if (context.Request.Query.TryGetValue("FEATURE_COUNT", out FEATURE_COUNTval))
                    {
                        fc = Convert.ToInt16(FEATURE_COUNTval);
                        if (fc < 1)
                            fc = 1;
                    }
                ...
                var infoFormat = INFO_FORMATval;
                string cqlFilter = null;
                if (context.Request.GetParam("CQL_FILTER") != null)
                {
                    cqlFilter = context.Request.GetParam("CQL_FILTER");
                }

                string vstr;
                var requestLayers = context.Request.GetParam("QUERY_LAYERS").Split(new[] { ',' });
                if (String.Compare(infoFormat, "text/json", ignoreCase) == 0)
                {
                    vstr = await CreateFeatureInfoGeoJSON(map, requestLayers, x, y, fc, cqlFilter, context);
                ...
                {
                    vstr = await CreateFeatureInfoPlain(map, requestLayers, x, y, fc, cqlFilter, context);
                    //string.Empty is the result if a WmsException.ThrowWmsException(...) has been called
                    if (vstr == string.Empty) return;
                ...
                await context.Response.WriteAsync(vstr);
                //context.Response.Flush();
                //context.Response.SuppressContent = true;
                //context.ApplicationInstance.CompleteRequest();
                ////context.Response.End();
            }
            else if (String.Compare(REQUESTVal.ToString(), "GetMap", ignoreCase) == 0) //Map requested
            {
                //Check for required parameters
                if (context.Request.GetParam("LAYERS") == null)
                {
                    await WmsException.ThrowWmsException("Required parameter LAYERS not specified", context);
                    return;
                }
                if (context.Request.GetParam("STYLES") == null)
                {
                    await WmsException.ThrowWmsException("Required parameter STYLES not specified", context);
                    return;
                }
                if (context.Request.GetParam("CRS") == null)
                {
                    await WmsException.ThrowWmsException("Required parameter CRS not specified", context);
                    return;
                }
                if (!ConsideredEqual(context.Request.GetParam("CRS"), $"EPSG:{map.Layers[0].TargetSRID}"))
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidCRS, "CRS not supported",
                                                   context);
                    return;
                }
                if (context.Request.GetParam("BBOX") == null)
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue,
                                                   "Required parameter BBOX not specified", context);
                    return;
                }
                if (context.Request.GetParam("WIDTH") == null)
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue,
                                                   "Required parameter WIDTH not specified", context);
                    return;
                }
                if (context.Request.GetParam("HEIGHT") == null)
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue,
                                                   "Required parameter HEIGHT not specified", context);
                    return;
                }
                if (context.Request.GetParam("FORMAT") == null)
                {
                    await WmsException.ThrowWmsException("Required parameter FORMAT not specified", context);
                    return;
                }
                ...
                if (String.Compare(context.Request.GetParam("TRANSPARENT"), "TRUE", ignoreCase) == 0)
                    map.BackColor = Color.Transparent;
                else if (context.Request.GetParam("BGCOLOR") != null)
                {
                    try
                    {
                        map.BackColor = ColorTranslator.FromHtml(context.Request.GetParam("BGCOLOR"));
                    }
                    catch
                    {
                        await WmsException.ThrowWmsException("Invalid parameter BGCOLOR", context);
                        return;
                    }
                }
                else
                ...
                ImageCodecInfo imageEncoder = GetEncoderInfo(context.Request.GetParam("FORMAT"));
                if (imageEncoder == null)
                {
                    await WmsException.ThrowWmsException("Invalid MimeType specified in FORMAT parameter", context);
                    return;
                }

                //Parse map size
                int width, height;
                if (!int.TryParse(context.Request.GetParam("WIDTH"), out width))
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue,
                ...
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.OperationNotSupported,
                                                   "Parameter WIDTH too large", context);
                    return;
                }
                if (!int.TryParse(context.Request.GetParam("HEIGHT"), out height))
                {
                    await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue,
                ...
                var bbox = ParseBBOX(context.Request.GetParam("bbox"), map.Layers[0].TargetSRID == 4326);
                if (bbox == null)
                {
                    await WmsException.ThrowWmsException("Invalid parameter BBOX", context);
                    return;
                ...
                if (String.IsNullOrEmpty(context.Request.GetParam("STYLES")))
                {
                ...
                {
                    if (!String.IsNullOrEmpty(context.Request.GetParam("LAYERS")))
                    {
                        var layerz = context.Request.GetParam("LAYERS").Split(new[] { ',' });
                        var styles = context.Request.GetParam("STYLES").Split(new[] { ',' });
                ...
                var cqlFilter = context.Request.GetParam("CQL_FILTER");
                ...
                //Set layers on/off
                var layersString = context.Request.GetParam("LAYERS");
                ...
                //If LAYERS is empty, use default layer on/off settings
                {
                    ...
                    foreach (var layer in layers)
                    {
                        ...
                        if (lay == null)
                        {
                            await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.LayerNotDefined,
            ...
            {
                await WmsException.ThrowWmsException(WmsException.WmsExceptionCode.OperationNotSupported, "Invalid request", context);
                return;
            }
        }
...
// ReSharper disable InconsistentNaming
        ...
        public static async Task<string> CreateFeatureInfoPlain(Map map, string[] requestedLayers, Single x, Single y, int featureCount, string cqlFilter, HttpContext context = null)
        {
        ...
        public static async Task<string> CreateFeatureInfoGeoJSON(Map map, string[] requestedLayers, Single x, Single y, int featureCount, string cqlFilterString, HttpContext context = null)
        {
            if (context == null)
                throw new InvalidOperationException("Cannot use CreateFeatureInfoGeoJSON without a valid HttpContext");

            var items = new List<GeoJSON>();
            foreach (var requestLayer in requestedLayers)
            {
                ...
                foreach (var mapLayer in map.Layers)
                {
                                      ...
                                      StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        var data = GeoJSONHelper.GetData(fds);
